//Estelle Brady
//CS 351 - 401
//Collaborated with: Miguel Garcia, Christian 
...
public abstract class Path {
	...
	/**
	 * Return the number of tasks on the path,
	 * which is one more than the path length,
	 * and is also positive (never zero).
	 ...
	 * @return a path that does the first and the continue with the next.
	 * The resulting path has one less task than the sum of the two
	 * because of the shared task.
	 */
	...
	/**
	 * Return whether this path represents a cycle:
	 * is not degenerate and has start and stop the same.
	 */
	...
	/**
	 * Some work that needs to be done later
	 * for a path at the given offset
	 */
	...
	/**
	 * Copy all the tasks of this path into a new array
	 * and return it.
	...
	public boolean contains(Task t) {
		...
		//loop through and add the pop to w
		while (!worklist.isEmpty()) {
			Work w = worklist.pop();
			if(w.path.containsHelper(worklist, t) == true) return true;
		}
		return false;
	}
	...
	public Task get(int index) {
		...
		while (!worklist.isEmpty()) {
			w = worklist.pop();
			b =w.path.getHelper(worklist, w.offset);
		}
		//return what we got for b
		return b;
	}
